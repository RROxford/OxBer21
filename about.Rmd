...
schedule_long <- schedule %>% 
  ...
  mutate(value = str_trim(value)) %>% # emoves whitespace from start and end of string
  left_join(course_info) %>% 
  mutate(value = if_else(!is.na(SpecificTime), str_c(value, SpecificTime, sep = " "), value)) # Join multiple strings into a single string
...
schedule_formatted <- schedule_long %>% 
  ...
  select(slot:value) %>% 
  fill(value) %>% # Fills missing values in selected columns using the previous entry
...
oxberlin_hux %>% 
  ...
  #set_valign(row = 1:3, col = 2:6, value="middle" ) %>% 
